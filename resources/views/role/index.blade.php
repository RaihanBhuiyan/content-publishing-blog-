                    ...
                    @foreach ($roles as $role)
                        <tr>
                            ...
                            <td data-label="Actions">
                                <a href="{{ route('roles.show', $role->id) }}" class="show"><i
                                        class="fa-solid fa-eye"></i></a>
                                @if (Auth::User()->roles[0]->id == $role->id)
                                    <a onClick="cannot('You cannot edit your own role!')" class="edit"><i
                                            class="fa-solid fa-pen-to-square"></i></a>
                                @else
                                    <a href="{{ route('roles.edit', $role->id) }}" class="edit"><i
                                            class="fa-solid fa-pen-to-square"></i></a>
                                @endif
                                @can('role-delete')
                                    @if (Auth::User()->roles[0]->name != $role->name)
                                        ...
                                        <button onClick="confirmDelete({{ $role->id }}, 'role')" class="delete"><i
